;; Casos de prueba de la especificación 05_Naive_Sort.md — GNU Guile + SRFI-64.
;;
...
(define (assert-sorts-all-cases algorithm sort-function)
  (test-begin algorithm)
  (for-each
   (lambda (test-case)
     (test-equal (string-append algorithm " should sort " (list-ref test-case 0))
                 (list-ref test-case 2)
                 (sort-function (list-ref test-case 1))))
   cases)
  (test-end algorithm))

(test-begin "naive_sort")

(assert-sorts-all-cases "selection_sort" selection-sort)
(assert-sorts-all-cases "bubble_sort" bubble-sort)
(assert-sorts-all-cases "insertion_sort" insertion-sort)
